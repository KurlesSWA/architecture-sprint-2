...
wait_for_mongo shard1_2 "$SHARD_PORT"
wait_for_mongo shard1_3 "$SHARD_PORT"
wait_for_mongo shard2 "$SHARD_PORT"
wait_for_mongo shard2_2 "$SHARD_PORT"
wait_for_mongo shard2_3 "$SHARD_PORT"
...
rs.initiate(
  {
    _id: "shard1rs",
    members: [
      { _id: 0, host: "mongodb_shard1:$SHARD_PORT" },
      { _id: 1, host: "mongodb_shard1_2:$SHARD_PORT" },
      { _id: 2, host: "mongodb_shard1_3:$SHARD_PORT" }
    ]
  }
)
EOF
...
rs.initiate(
  {
    _id: "shard2rs",
    members: [
      { _id: 0, host: "mongodb_shard2:$SHARD_PORT" },
      { _id: 1, host: "mongodb_shard2_2:$SHARD_PORT" },
      { _id: 2, host: "mongodb_shard2_3:$SHARD_PORT" },
    ]
  }
)
EOF
